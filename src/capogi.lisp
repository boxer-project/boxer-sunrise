...
(defun test-draw-capogi-char (char x y)
  (drawing-on-window (*boxer-pane*)
                     (bw::gl-pixel-storei bw::*gl-unpack-alignment* 1)
                     (bw::gl-raster-pos2-i x y)
                     (fli:with-dynamic-foreign-objects ((bm (:unsigned :byte)
                                                            :nelems (length (bw::capogi-char-data char))
                                                            :initial-contents (bw::capogi-char-data char)))
                       (bw::gl-bitmap (bw::capogi-char-wid char) (bw::capogi-char-hei char) 0.0 0.0 0.0 0.0 bm)))
    ...
    (do* ((chars (capogi-font-chars font))
      ...
      (let ((char (svref chars (char-code (char string i)))))
        (fli:with-dynamic-foreign-objects ((bm (:unsigned :byte)
                                               :nelems (length (bw::capogi-char-data char))
                                               :initial-contents (bw::capogi-char-data char)))
           (bw::gl-bitmap (bw::capogi-char-wid char) (bw::capogi-char-hei char) 0.0 0.0
