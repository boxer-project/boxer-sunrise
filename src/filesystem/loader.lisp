...
(defun load-8-bit-run-length-encoded-pixmap (stream)
  ...
  (let ((width (bin-next-value stream)) (height (bin-next-value stream))
    ...
    ;; now render the data
    (let* ((pixmap (make-ogl-pixmap width height))
           (x 0) (y 0))
      (loop
          (let* ((word (bin-next-byte stream))
            ...
            (dotimes& (i count)
              (set-pixmap-pixel pixmap x y pixel)
              (incf& x)
...
(defun load-true-color-rgba-pixmap (stream)
  (let* ((width (bin-next-value stream))
         ...
         (pixmap (make-ogl-pixmap width height))
         (x 0) (y 0))
    ...
    (loop
        (let* ((count (bin-next-byte stream))
          ...
          (dotimes& (i count)
            (setf (ldb (byte 16 16) (pixmap-pixel pixmap x y)) 1st-word)
            (setf (ldb (byte 16 0) (pixmap-pixel pixmap x y)) 2nd-word)
            (incf& x)
            (when (>=& x width) (setq x 0 y (1+& y))))
          (when (>=& y height) (return))))
    pixmap))
...
(defun load-true-color-run-length-encoded-pixmap (stream)
  (let* ((width (bin-next-value stream)) (height (bin-next-value stream))
         ...
         (true-color? (>= (ogl-pixmap-depth pixmap) 16.))
         (x 0) (y 0))
    (declare (fixnum width height x y))
    (loop
        (let* ((1st-word (bin-next-byte stream))
               ...
               (pixel (if true-color?
                        (make-offscreen-pixel red green blue)
        ...
        (drawing-on-bitmap (pixmap)
          (dotimes& (row height)
            (dotimes& (rb whole-words-per-row)
              (put-picture-byte pixmap (ash& rb 4) row (get-bits) 8.)
              (put-picture-byte pixmap (+& (ash& rb 4) 8.) row (get-bits) 8.))
            ;; handle any protrusions
            (when (not (zerop& leftover-pixels))
              (put-picture-byte pixmap (ash& whole-words-per-row 4)
              ...
              (if (<& leftover-pixels 8.)
                (get-bits)
                (put-picture-byte pixmap (+& (ash& whole-words-per-row 4) 8)
...
(defmethod partial-initialize ((self graphics-object) keyword value)
  (case keyword
    ...
    (z-position (setf (slot-value self 'z-position)
                      (%make-vv-box-interface value 'z-position)))
    ...
    (do* ((list plist (cddr list))
      ...
      (cond ((and (not (null *check-for-and-convert-old-vlist-style*))
            ...
            ;;         (update-save-under turtle)))
             nil)
            ((fast-memq slot
                        '(x-position y-position z-position heading pen-width sprite-size))
             (setf (slot-value turtle slot)
