...
#|
 ...
 Copyright 1998 - 2013 PyxiSystems LLC
...
;; Now with registration and expiration !!
(defun window-system-specific-start-boxer ()
  (cond ((boxer::valid-boxer-license?)
         (window-system-specific-start-boxer-1))
        ((boxer::demo-boxer-license?)
         (cond ((boxer::expired-boxer?)
                (capi:display-message "The Free Trial Period for this Edition has expired.  Enter a Valid License Key to Continue using Boxer")
                (valid-key-or-quit))
               (t (boxer::demo-next-day)
                  (window-system-specific-start-boxer-1))))
...
(defun boxer-expose-window-handler (pane x y wid hei)
  ...
  (cond ((not (null *display-bootstrapping-no-boxes-yet*))
        ...
        (t
         (rendering-on (pane) (ogl-init wid hei))  ;(ogl-reshape wid hei)
         (redraw-status-line)
         (boxer::force-repaint))))
