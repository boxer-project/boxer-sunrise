...
(defun translate-graphics-command (graphics-command trans-x trans-y)
  (if (< (aref graphics-command 0) 32)
    (error "Translate-graphics-command with older command: ~A" graphics-command)
    (let* ((com (gethash (aref graphics-command 0) *graphics-commands*))
      ...
      (unless (null template)
        (dolist (item template)
          (case item
            (:x-transform (setf (aref togo idx) (+ trans-x (aref togo idx))))
            (:y-transform (setf (aref togo idx) (+ trans-y (aref togo idx))))
            (:coerce nil)
            (t nil))
          (incf idx)))
      togo)))
